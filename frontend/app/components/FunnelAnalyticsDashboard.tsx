...
export function AnalyticsDashboard({ platform, metrics, className = '' }: AnalyticsDashboardProps) {
  ...
  const platformColors: Record<string, { primary: string; secondary: string }> = {
    twitter: { primary: '#1DA1F2', secondary: '#E1E8ED' },
    linkedin: { primary: '#0A66C2', secondary: '#E7E5E4' },
    facebook: { primary: '#1877F2', secondary: '#E4E6EB' },
    instagram: { primary: '#E4405F', secondary: '#FAFAFA' },
    tiktok: { primary: '#25F4EE', secondary: '#EFF7FF' },
  }
        ...
        <CardContent className="space-y-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 pt-6 border-t">
            ...
            <div className="space-y-2">
              ...
              <p className="text-lg font-bold">
                {(
                  100 - (funnelData[funnelData.length - 1]?.value / funnelData[0]?.value) * 100
                ).toFixed(1)}
                %
              </p>
            </div>
